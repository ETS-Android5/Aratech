...
class Home extends React.Component {
	...
	async componentDidMount() {
		...
		const upcomingEvents = this.state.events.filter(
			(event) => event.startTime - Date.now() < 10800000 && event.startTime - Date.now() > 0
		);
		this.setState({
			upcomingEvents,
		});
		...
		const onGoingEvents = this.state.events.filter(
			(event) => event.startTime - Date.now() <= 0 && event.endTime - Date.now() > 0
		);
		this.setState({
			onGoingEvents,
		});
	...
	render() {
		const { events, upcomingEvents, onGoingEvents, image, uploading, isLoading, currentEvent, show } = this.state;
		return (
			<React.Fragment>
				<Navbar />
				{/**calendar component */}
				<div className="uk-grid-collapse" data-uk-grid>
					...
					<div className="uk-width-1-2@m uk-padding-large uk-flex uk-flex-column">
						<div className="uk-card uk-card-default uk-card-hover uk-card-body uk-text-center">
							...
							{isEmpty(upcomingEvents) ? <p>No Upcoming Events now...</p> : null}
							<ul>
									...
									<div className="uk-width-1-1 uk-margin">
										...
										<input
											...
											className={`uk-input uk-form-large ${
												touched.events && errors.eventName ? 'uk-form-danger' : null
											}`}
											type="text"
										...
										<label className="uk-form-label uk-margin-small-right" htmlFor="startTime">
											Start Time:
										</label>
										<DateTimePicker
											id="startTime"
											...
											className={`uk-margin-top ${
												touched.startTime && errors.startTime ? 'uk-form-danger' : null
											}`}
											onChange={(date) => {
              ...
              Accepted image types are .png and .jpg"
				...
				{currentEvent ? (
					...
					>
						{`${currentEvent.eventName} starts at ${moment(currentEvent.startTime).format(
							'Do MMMM YYYY, h:mm a'
						)} and ends at ${moment(currentEvent.endTime).format('Do MMMM YYYY, h:mm a')}`}
					</SweetAlert>
				) : null}
			</React.Fragment>
		);
...
const mapStateToProps = ({ auth: { user }, timetable: { personalTimetable, classTimetable } }) => ({
	user,
	personalTimetable,
	classTimetable,
});
